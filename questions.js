...
*/
//typeof() ->renvoie le type de la chaine
...
//isNaN() détermine si la variable passée en paramètre est un nombre ou pas
...
var AfficherExtensionString = function (texte) {
  //on donne à substring() l'indice équivalent à la dernière occurence "." auquel on a ajouté la valeur 1
  return texte.substring(texte.lastIndexOf('.')+1);
}
/*var AfficherExtensionString = function (texte) {
return texte.split('.').slice(-1).toString();
}
*/
//texte.split(séparateur) -> permet de séparer une chaine en plusieurs éléments
var NombreEspaceString = function (texte) {
  //split(' ') met chaque sur une ligne, donc -1 avec le legnth
...
var calculIMC = function (poids, taille) {
  var cal = poids / Math.pow(taille, 2);
  cal = cal.toFixed(2);
